...
function fail(message) {
  process.stderr.write(`model-dispatch-router config error: ${message}\n`);
  process.exit(2);
}

let project = {};
const configPath = process.env.MODEL_DISPATCH_ROUTER_CONFIG || process.env.MODEL_DISPATCH_ROUTE_CONFIG || path.join(repoRoot, '.model-dispatch-router.json');
