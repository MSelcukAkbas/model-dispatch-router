...
@app.command()
def snapshot(
    source: Path = typer.Argument(..., help="Live model-dispatch-router repo to read."),
    ...
    db, repo = _open(root)
    snapshots_dir = db.path.parent / "snapshots"
    db.close()
    result = take_snapshot(source, snapshots_dir, with_logs=with_logs)
...
@app.command("import")
def import_cmd(
    snapshot_dir: Path = typer.Argument(..., help=".agentmind/snapshots/<stamp> directory."),
    root: Path = typer.Option(None, "--root", help="Workspace root (default: nearest .agentmind above cwd)."),
) -> None:
...
def tasks(
    ...
    for r in rows:
        table.add_row(
            ...
            _fmt(r["engine"]),
            _fmt(r["model"]),
            _fmt(r["account"]) or "current-claude-session",
            _fmt(r["effort"]),
            _fmt(r["budget_usd"]),
            _fmt(r["attempt"]),
            _fmt(r["diffstat"]),
...
def gate(
    ...
    if result.passed:
        ...
        dirty_check = next((c for c in result.checks if c.name == "evidence_clean"), None)
        if task_row and task_row["no_worktree"] and dirty_check and not dirty_check.ok:
            console.print(
                "[yellow]no-worktree task[/yellow]: it ran directly in the main "
                "checkout with no isolated copy to diff against, so verification is "
                "blocked by the uncommitted scoped file(s) above, not passed. Commit "
                "or clean the scope, then re-run gate."
            )
        raise typer.Exit(1)
    db.close()
...
@app.command()
def setup(
    source: Path = typer.Argument(..., help="Live model-dispatch-router repo to learn from."),
    ...
    snap = take_snapshot(source, Path(target) / ".agentmind" / "snapshots")
