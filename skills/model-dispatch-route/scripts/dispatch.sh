...
#                     explore). Must match .claude/skills/model-dispatch-route/personas/<role>.md.
...
# Optional project configuration. A checked-in `.model-dispatch-route.json` can
...
if [ "${ROLE_BRIDGE[$ROLE]:-false}" != "true" ]; then
  ...
  if [ "$ROLE" = "ops" ]; then
    # opsMcpConfig is opt-in per repo (.model-dispatch-route.json) since remote
    ...
    echo "warning: role=ops but no opsMcpConfig is set in .model-dispatch-route.json — dispatching with NO remote-access MCP server (bridge only). Set opsMcpConfig if this task needs infrastructure tools." >&2
  fi
