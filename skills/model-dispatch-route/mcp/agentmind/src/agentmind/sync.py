...
def _prune_snapshots(snapshots_dir: Path, keep: int = 3) -> None:
    """Retain only the latest N snapshots to prevent unbounded disk accumulation."""
    if not snapshots_dir.is_dir():
        return
    subdirs = sorted([d for d in snapshots_dir.iterdir() if d.is_dir()], key=lambda d: d.name)
    while len(subdirs) > keep:
...
def sync(
    ...
    *,
    ...
    snap = take_snapshot(src, Path(workspace) / ".agentmind" / "snapshots")
    imported = import_snapshot(repo, snap.dest, corpus=name)
    _prune_snapshots(snap.dest.parent, keep=3)
