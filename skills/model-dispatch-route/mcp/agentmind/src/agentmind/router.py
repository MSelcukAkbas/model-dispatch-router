...
* It makes the existing policy explicit. The rules already live in
  model-dispatch-route's SKILL.md and in its ROLE_MODEL/ROLE_EFFORT arrays, as prose
...
# From model-dispatch-route/SKILL.md and dispatch.sh's ROLE_MODEL / ROLE_EFFORT /
...
def gather_evidence(repo: Repository, role: str, engine: str | None = None) -> Evidence:
    ...
    for row in rows:
        if row["attempt"] is not None:
            ...
            if row["attempt"] > 1:
                ev.retried += 1
        account = row["account"] or ("n/a" if row["engine"] == "agy" else "current-claude-session")
