"""Automatic AgentMind lifecycle around a model-dispatch run.
...
from .workspace import locate_store
...
def _open_runtime(
    ...
    if root is not None:
        ...
        workspace, _ = locate_store(source)
        if workspace is None:
            raise RuntimeError("no AgentMind store found for this repository")
    db = Database(resolve_db_path(workspace))
    db.init_db()
    return workspace, db, Repository(db)


def _corpus(repo: Repository, name: str | None) -> str:
    ...
    if row is None:
        ...
        raise RuntimeError("several AgentMind corpora exist; pass repo_name")
    return str(row["name"])
...
def session_start(
    ...
    *,
    ...
    workspace, db, repo = _open_runtime(src, root)
