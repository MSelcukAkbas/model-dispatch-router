#!/bin/bash
# Usage: dispatch.sh <role> <task-id> <prompt-file> [timeout-minutes]
...
#   timeout-minutes  optional, default 25.
...
#   budget-usd       optional, human-supplied override for --max-budget-usd.
...
case "$NO_WORKTREE" in
  0|1) ;;
  *) echo "error: invalid no-worktree flag '$NO_WORKTREE' — must be 0 or 1" >&2; exit 1 ;;
esac
ACCOUNT="${8:-}"
...
" 2>/dev/null)"
  if [ -n "$QUOTA_PCT" ] && awk -v p="$QUOTA_PCT" 'BEGIN{exit !(p>=90)}'; then
    echo "refused: account=${ACCOUNT:-ambient} session quota at ${QUOTA_PCT}% — too high to safely dispatch. Resets in ~${QUOTA_RESET_MIN} minute(s). Call ScheduleWakeup for ~$((QUOTA_RESET_MIN + 2)) minutes and retry then — do not dispatch now. If another account has headroom, dispatch there instead (account=<name>) rather than waiting on this one." >&2
    exit 15
  fi
else
  echo "warning: could not check quota for account=${ACCOUNT:-ambient} before dispatch (usage.sh failed) — proceeding anyway, watch manually." >&2
fi
...
if [ -n "$FILES_LINE" ]; then
  THIS_FILES="$(echo "${FILES_LINE#"# FILES:"}" | tr -d ' ')"
  for other in "$LOG_DIR"/*.files; do
    [ -e "$other" ] || continue
    OTHER_TASK="$(basename "$other" .files)"
    [ "$OTHER_TASK" = "$TASK" ] && continue
    [ -d "$REPO_ROOT/.worktrees/$OTHER_TASK" ] || continue
    OTHER_FILES="$(cat "$other")"
    IFS=',' read -ra MINE <<< "$THIS_FILES"
    for f in "${MINE[@]}"; do
      if echo ",$OTHER_FILES," | grep -qF ",$f,"; then
        echo "refused: file '$f' overlaps with still-active task '$OTHER_TASK'." >&2
        exit 12
      fi
    done
  done
  echo "$THIS_FILES" > "$LOG_DIR/$TASK.files"
fi
...
if [ "$(cd "$(dirname "$PROMPT_FILE")" && pwd)/$(basename "$PROMPT_FILE")" != "$LOG_DIR/$TASK.prompt.orig" ]; then
...
if [ -f "$LOG_DIR/$TASK.pid" ] && [ ! -f "$LOG_DIR/$TASK.exitcode" ]; then
  OLD_PID="$(cat "$LOG_DIR/$TASK.pid" 2>/dev/null)"
  if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
    echo "error: task '$TASK' already has a live run in progress (pid $OLD_PID) — refusing to start a second one over it. Wait for it (status.sh/wait.sh), or resume.sh it once it reaches a terminal state." >&2
    exit 1
  fi
fi
TS_STALE="$(date +%s)"
...
for ext in json err exitcode pid bridge.json stophook-count result-missing; do
...
(
  ...
  cd "$RUN_DIR" || { echo "cd to RUN_DIR ($RUN_DIR) failed — worktree may have been removed concurrently" >> "$LOG_DIR/$TASK.err"; echo 1 > "$LOG_DIR/$TASK.exitcode"; exit 1; }
  ...
  RUN_EXIT="$?"
  echo "$RUN_EXIT" > "$LOG_DIR/$TASK.exitcode"
  if command -v am >/dev/null 2>&1 && [ "${AGENTMIND_DISABLE:-0}" != "1" ]; then
    case "$RUN_EXIT" in
    ...
    am session-end "$TASK" \
      ...
      > "$LOG_DIR/$TASK.agentmind-end.log" 2>&1 || true
  fi
) &
...
if [ "${ROLE_READONLY[$ROLE]:-false}" = "true" ] || [ "$NO_WORKTREE" = "1" ]; then
  echo "dispatched: task=$TASK role=$ROLE model=$MODEL effort=$EFFORT budget=\$$MAX_BUDGET attempt=$ATTEMPT account=${ACCOUNT:-ambient} run_dir=$RUN_DIR(main checkout, no worktree) pid=$DISPATCH_PID timeout=${TIMEOUT_MIN}m"
else
  echo "dispatched: task=$TASK role=$ROLE model=$MODEL effort=$EFFORT budget=\$$MAX_BUDGET attempt=$ATTEMPT account=${ACCOUNT:-ambient} worktree=$WORKTREE_DIR branch=$BRANCH pid=$DISPATCH_PID timeout=${TIMEOUT_MIN}m"
fi
exit 0
