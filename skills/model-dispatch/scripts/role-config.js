...
const defaults = {
  ...
  judge:    { model: 'opus',   effort: 'high',   readonly: true,  bridge: true, maxBudgetUsd: 4.00 },
};

function fail(message) {
  process.stderr.write(`model-dispatch config error: ${message}\n`);
  process.exit(2);
}

let project = {};
const configPath = process.env.MODEL_DISPATCH_CONFIG || path.join(repoRoot, '.model-dispatch.json');
