#!/bin/bash
# Usage: resume.sh <task-id> [timeout-minutes] [effort] [budget-usd]
#
...
STATUS_CODE=$?
if [ "$STATUS_CODE" != "17" ] && [ "$STATUS_CODE" != "14" ] && [ "$STATUS_CODE" != "20" ]; then
...
exec bash "$SCRIPT_DIR/dispatch.sh" "$ROLE" "$TASK" "$LOG_DIR/$TASK.prompt.orig" "$TIMEOUT_MIN" "$EFFORT" "$BUDGET_OVERRIDE" "${META_NO_WORKTREE:-0}" "$META_ACCOUNT"
