 ...
 *
 * `skills/model-dispatch-route` is the only editable implementation.  The output is
...
const sourceSkill = join(root, "skills", "model-dispatch-route");
const packageTemplate = join(root, "packaging", "agentmind-model-dispatch-route");
const output = join(root, "dist", "agentmind-model-dispatch-route");
...
cpSync(sourceSkill, join(output, "skills", "model-dispatch-route"), {
  recursive: true,
...
cpSync(join(packageTemplate, "examples", "model-dispatch-route.example.json"), join(output, "examples", "model-dispatch-route.example.json"));

const manifestPath = join(output, ".codex-plugin", "plugin.json");
const manifest = JSON.parse(readFileSync(manifestPath, "utf8"));
if (manifest.name !== "agentmind-model-dispatch-route" || manifest.skills !== "./skills/") {
