...
function fixture(t) {
  ...
  return {
    dir,
    put: (suffix, value) => fs.writeFileSync(path.join(dir, `T-1.${suffix}`), typeof value === 'string' ? value : JSON.stringify(value))
  };
}
...
test('checkpoint recovers last assistant even when final event is a tool event', t => {
  const { dir, put } = fixture(t);
  put('json', [
    JSON.stringify({ type: 'assistant', message: { content: [{ type: 'text', text: 'Verified A; B still pending' }] } }),
...
test('AGY partial protocol is surfaced as partial', t => {
  const { dir, put } = fixture(t);
  put('agy.exitcode', '0');
  put('agy.json', { response: 'STATUS: BLOCKED\nMissing input' });
  assert.equal(status(dir, 'T-1').exit_code, 22);
});
...
test('Codex success parses JSONL thread_id, agent_message, and output-last-message', t => {
  const { dir, put } = fixture(t);
  const jsonl = [
    JSON.stringify({ type: 'thread.started', thread_id: 'th_test_12345' }),
    JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'All tasks completed successfully' } })
  ].join('\n');
  put('codex.json', jsonl);
  put('codex.exitcode', '0');
  put('codex.meta', 'engine=codex\ntask=T-1\n');

  assert.equal(status(dir, 'T-1').exit_code, 0);

  const final = finalize(dir, 'T-1', 0);
  assert.equal(final.exit_code, 0);
  assert.equal(final.thread_id, 'th_test_12345');

  const metaContent = fs.readFileSync(path.join(dir, 'T-1.codex.meta'), 'utf8');
  assert.match(metaContent, /thread_id=th_test_12345/);

  hook(dir, 'T-1', 0);
  const col = collect(dir, 'T-1');
  assert.equal(col.exit_code, 0);
  assert.match(col.report, /All tasks completed successfully/);
});

test('Codex partial and blocked statuses are surfaced correctly', t => {
  const { dir, put } = fixture(t);
  const jsonl = [
    JSON.stringify({ type: 'thread.started', thread_id: 'th_partial_1' }),
    JSON.stringify({ type: 'item.completed', item: { type: 'agent_message', text: 'STATUS: PARTIAL\nNeed further confirmation' } })
  ].join('\n');
  put('codex.json', jsonl);
  put('codex.exitcode', '0');

  assert.equal(status(dir, 'T-1').exit_code, 22);
  assert.equal(status(dir, 'T-1').status, 'partial');
});

test('Codex errors and quota exhaustion are classified appropriately', t => {
  const { dir, put } = fixture(t);
  put('run.json', { engine: 'codex' });
  const errJsonl = [
    JSON.stringify({ type: 'turn.failed', error: { message: 'rate_limit_error: quota exceeded' } })
  ].join('\n');
  put('codex.json', errJsonl);
  put('codex.exitcode', '1');

  assert.equal(status(dir, 'T-1').exit_code, 17);
  assert.equal(status(dir, 'T-1').status, 'quota_exhausted');
});
