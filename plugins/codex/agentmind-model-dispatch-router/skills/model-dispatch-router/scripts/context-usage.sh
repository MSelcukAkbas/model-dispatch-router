...
#   account      optional name from a local account registry — look under THAT
#                account's config dir instead of the current session. Default "":
#                current session (whatever
...
if [ -z "$FILE" ]; then
  echo "Session transcript not found for $SESSION_ID under $CONFIG_DIR/projects (account=${ACCOUNT:-current-claude-session}) — wrong ID, wrong account, or transcript not yet written." >&2
  exit 1
fi
