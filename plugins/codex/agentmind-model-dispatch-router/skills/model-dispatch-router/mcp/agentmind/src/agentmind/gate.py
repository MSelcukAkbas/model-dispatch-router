...
def evaluate_task(
    ...
    *,
    ...
    no_worktree = bool(task_row["no_worktree"]) if task_row else False
    if not dirty:
        clean_detail = "cited files match HEAD"
    elif no_worktree:
        # A no-worktree task ran directly in the main checkout, so "dirty" here
        # can mean this task's own uncommitted work, unrelated concurrent
        # changes, or both — there is no isolated copy to tell them apart.
        # Say so plainly: this is a block, not a partial pass, and the claims
        # stay candidates either way.
        clean_detail = (
            f"BLOCKED (no-worktree task): {len(dirty)} scoped file(s) uncommitted "
            f"— {', '.join(dirty[:3])}. Commit or clean the scope, then re-run gate. "
            "Claims remain candidates; this is not a pass."
        )
    else:
        clean_detail = f"{len(dirty)} with uncommitted changes: {', '.join(dirty[:3])}"
    result.checks.append(Check("evidence_clean", not dirty, clean_detail))
