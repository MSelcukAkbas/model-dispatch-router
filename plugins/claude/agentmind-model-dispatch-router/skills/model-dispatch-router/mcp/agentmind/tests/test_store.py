...
def test_task_stats_separates_agy_from_current_claude_session(tmp_path):
    """An agy .meta has no account field. Without the engine split its rows
    would group under 'current-claude-session' and be counted as Claude dispatches."""
    db, repo = _repo(tmp_path)
    ...
    assert rows == {("agy", "n/a"): 1, ("claude", "current-claude-session"): 1, ("claude", "secondary"): 1}
    db.close()
